...
import { FileText, ArrowRight, Loader2, AlertTriangle, CheckCircle, PenLine, Wand2 } from 'lucide-react'
...
export default function ScriptGenerationStep() {
  ...
  // Manual multi-scripts state
  const [manualMultiScripts, setManualMultiScripts] = useState<string[]>(() =>
    approvedIdeas.map((_, i) => multiScripts[i]?.script ?? '')
  )
  ...
  async function handleGenerate() {
    logUIEvent('click:script:generate', { wordCount, isMulti, ideaCount: approvedIdeas.length })
    setLoading(true)
    setError('')
    setScriptMeta(null)
    setMultiResults([])
  ...
  function handleUseScript() {
    ...
    logUIEvent('click:script:use', { wordCount: generatedScript.split(/\s+/).length })
  ...
  function handleUseManualScript() {
    if (!generatedScript.trim()) return
    logUIEvent('click:script:useManual', { wordCount: generatedScript.split(/\s+/).length })
    if (promoteTimerRef.current) clearTimeout(promoteTimerRef.current)
    setPromotingIndex('single')
    promoteTimerRef.current = setTimeout(() => {
      setPromotingIndex(null)
  ...
  function handleUseManualMultiScripts() {
    const scripts = manualMultiScripts.map((script, i) => ({
      ideaTitle: approvedIdeas[i]?.title ?? `Idea ${i + 1}`,
      ideaDescription: approvedIdeas[i]?.description ?? '',
      script,
  ...
  const canUseManualMulti = manualMultiScripts.some(s => s.trim().length > 0)
        ...
        <>
          ...
          <div className="mt-4 flex items-end gap-4">
            <div className="flex-1">
              ...
              <input
                type="number"
                value={wordCountRaw}
                onChange={(e) => setWordCountRaw(e.target.value)}
            ...
            <div className="pb-1">
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Accepted range</p>
              <p className="text-xs font-black text-slate-600">{low} – {high} words</p>
            </div>
          </div>
                  ...
                  <div key={i} className={`rounded-2xl border bg-white overflow-hidden shadow-sm transition-all ${isSelected ? 'border-emerald-300 ring-2 ring-emerald-50' : 'border-slate-100'}`}>
                    <div className="px-4 py-3 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
                      ...
                      <span className={`ml-3 shrink-0 flex items-center gap-1.5 text-[10px] font-bold px-2.5 py-1 rounded-full ${
                        result.lengthOk ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'
                      }`}>
                        {result.lengthOk
          ...
          {!isMulti && generatedScript && (
            <div className="mt-6">
              ...
              >
                {promotingIndex === 'single' ? (
                  <><CheckCircle size={16} strokeWidth={2.5} />Sent to Step 4</>
                ) : (
        ...
        <>
          {isMulti ? (
            <>
              <div className="mb-4 px-4 py-3 rounded-xl bg-violet-50 border border-violet-100">
                <p className="text-xs font-bold text-violet-700">
                  Enter one script per idea. Each line in the script = one scene in Step 4.
                </p>
              </div>

              <div className="space-y-5">
                {approvedIdeas.map((idea, i) => {
                  const wc = manualMultiScripts[i]?.split(/\s+/).filter(Boolean).length ?? 0
                  return (
                    <div key={i} className="rounded-2xl border border-slate-100 bg-white overflow-hidden shadow-sm">
                      <div className="px-4 py-3 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
                        ...
                        {wc > 0 && (
                          <span className="ml-3 shrink-0 text-[10px] font-bold px-2.5 py-1 rounded-full bg-slate-100 text-slate-500">
                            {wc}w
                          </span>
                        )}
                      </div>
                      <div className="p-4">
                        <textarea
                          value={manualMultiScripts[i] ?? ''}
                          onChange={(e) => {
                            const next = [...manualMultiScripts]
                            next[i] = e.target.value
                            setManualMultiScripts(next)
                          }}
                          placeholder="Type or paste the script here. Each line = one scene."
                          rows={8}
                          className="w-full px-4 py-3 rounded-xl text-sm font-medium bg-slate-50 border border-slate-100 text-slate-800 outline-none resize-y focus:border-emerald-400 focus:ring-4 focus:ring-emerald-50 transition-all leading-relaxed placeholder:text-slate-300"
                        />
                      </div>
                    </div>
                  )
              ...
              <button
                onClick={handleUseManualMultiScripts}
                disabled={!canUseManualMulti}
                className="mt-5 flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-black text-emerald-700 bg-emerald-50 border-2 border-emerald-100 hover:bg-emerald-100 transition-all disabled:opacity-30"
              >
                Use These Scripts
                <ArrowRight size={16} />
              </button>
            ...
            <>
              ...
              <p className="text-[11px] text-slate-400 font-medium mb-3">
                Each line = one scene. Step 4 will process each line separately.
              </p>
              <textarea
                value={generatedScript}
                onChange={(e) => setGeneratedScript(e.target.value)}
                placeholder="Type or paste your full script here. Each line becomes one scene…"
                rows={12}
                className="w-full px-4 py-3 rounded-xl text-sm font-medium bg-slate-50 border border-slate-100 text-slate-800 outline-none resize-y focus:border-emerald-400 focus:ring-4 focus:ring-emerald-50 transition-all leading-relaxed placeholder:text-slate-300"
              />
              {generatedScript.trim() && (
                <p className="mt-2 text-[11px] text-slate-400 font-medium">
                  {generatedScript.trim().split(/\s+/).filter(Boolean).length} words · {generatedScript.trim().split('\n').filter(s => s.trim()).length} scenes
                </p>
              )}
              <button
                onClick={handleUseManualScript}
                disabled={!generatedScript.trim() || promotingIndex !== null}
                className={`mt-3 flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-black transition-all disabled:opacity-30 ${
                  promotingIndex === 'single'
                    ? 'text-white bg-emerald-500 border-2 border-emerald-500'
                    : 'text-emerald-700 bg-emerald-50 border-2 border-emerald-100 hover:bg-emerald-100'
                }`}
